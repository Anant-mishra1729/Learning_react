    ...
    return (
        <>
            <div>
                ...
                <button className="btn btn-primary mx-1" id="toUpper" onClick={handleUpClick}>Convert to uppercase</button>
                <button className="btn btn-primary mx-1" id="toUpper" onClick={handleLowClick}>Convert to lowercase</button>
            </div>

            <div className="container my-2">
                <h1>Summary</h1>
                <p>{text.split(" ").length} words and {text.length} characters</p>
                <p>Reading time : <span>{text.split(' ').length * 0.008}</span></p>
            </div>
        </>
    )
}
